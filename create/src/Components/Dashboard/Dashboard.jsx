          ...
          <div className="custom-modal-content">
            ...
            <form onSubmit={handleSaveBudget}>
              <div className="mb-3">
                ...
                <input type="text" className="form-control" value={budgetName} onChange={e => setBudgetName(e.target.value)} />
              </div>
              {categories.map((cat, idx) => (
                <div className="category-row" key={idx} style={{ display:'flex', gap:'5px', alignItems:'center', marginBottom:'10px' }}>
                  <input type="text" placeholder="Category Name" value={cat.name} onChange={e => handleCategoryChange(idx, 'name', e.target.value)} />
                  <input type="number" placeholder="Amount" value={cat.amount} onChange={e => handleCategoryChange(idx, 'amount', e.target.value)} />
                  <input type="number" placeholder="Percent" value={cat.percent} onChange={e => handleCategoryChange(idx, 'percent', e.target.value)} min={0} max={100} />
                  <button type="button" onClick={() => removeCategory(idx)}>–</button>
                </div>
              ))}
              <button type="button" onClick={addCategory} style={{ marginBottom: '10px' }}>+</button>
